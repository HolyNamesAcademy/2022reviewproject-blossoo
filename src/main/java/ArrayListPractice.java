...
public class ArrayListPractice {
    ...
    public static Student GetTallestStudent(ArrayList<Student> students) {
        Student tall = students.get(0);
        int previousH = students.get(0).GetHeight();
        for (int i = 1; i < students.size(); i++) {
            int currentH = students.get(i).GetHeight();
    ...
    public static ArrayList<Student> GetStudentsInGradeLevel(ArrayList<Student> students, int gradeLevel) {
        ArrayList<Student> gradeL = new ArrayList<>();
        for (Student student : students) {
    ...
    public static boolean TransferMoney(ArrayList<Student> students, String fromStudentName, String toStudentName, double amount) {
        int accBalS1 = 0;
        int accBalS2 = 0;
        //set how much money FromSN and toSN have
        ...
        if(students.get(accBalS1).GetBankAccount().Withdraw(amount) != 0) {
            students.get(accBalS2).GetBankAccount().Deposit(amount);
            return true;
        }
        return false;
    }
